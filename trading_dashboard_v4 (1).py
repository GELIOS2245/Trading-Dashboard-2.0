...
def _normalize_hist(df):
    """
    Robustly flatten any DataFrame from yf.download() regardless of
    yfinance version or Python version (3.10-3.14). Handles MultiIndex,
    tuple columns, duplicate columns, and Series-wrapped-in-DataFrame.
    Returns a flat-column DataFrame or empty DataFrame.
    """
    if df is None or (hasattr(df, "empty") and df.empty):
        return pd.DataFrame()
    # Flatten MultiIndex — yfinance sometimes returns (metric, ticker) tuples
    if isinstance(df.columns, pd.MultiIndex):
        df = df.copy()
        df.columns = df.columns.get_level_values(0)
    # Normalise column names to capitalised strings
    new_cols = []
    for c in df.columns:
        if isinstance(c, tuple):
            new_cols.append(str(c[0]).strip().capitalize())
        else:
            new_cols.append(str(c).strip().capitalize())
    df = df.copy()
    df.columns = new_cols
    # Drop duplicate column names (MultiIndex flattening can produce them)
    df = df.loc[:, ~df.columns.duplicated()]
    # Squeeze any column that is itself a DataFrame (edge case in yfinance 1.x)
        ...
        for attempt in range(3):
            try:
                ...
                if not df.empty and "Close" in df.columns:
                    # Ensure Close is a plain float Series, not a DataFrame
                    close_col = df["Close"]
                    if isinstance(close_col, pd.DataFrame):
                        close_col = close_col.iloc[:, 0]
                    if len(close_col.dropna()) > 5:
                        df["Close"] = close_col.astype(float)
                        return df
            except Exception:
                pass
            try:
                # Fallback: no multi_level_index kwarg (older yfinance compat)
                df = yf.download(t, period=period, interval="1d",
...
def compute_technicals(hist: pd.DataFrame) -> dict:
    ...
    if hist is None or hist.empty or "Close" not in hist.columns:
        return t
    def _to_series(col):
        """Safely extract a plain float Series from a column that might be a DataFrame."""
        s = hist[col] if col in hist.columns else hist["Close"]
        if isinstance(s, pd.DataFrame): s = s.iloc[:, 0]
        return s.astype(float)
    close = _to_series("Close")
    high  = _to_series("High")  if "High"  in hist.columns else close
    low   = _to_series("Low")   if "Low"   in hist.columns else close
    n     = len(close)
...
def render_deep_dive(ticker: str, bias_override: str = "AUTO", key_prefix: str = "dd"):
    ...
    if hist is not None and not hist.empty and "Close" in hist.columns:
        try:
            ...
            lc = float(close_s.astype(float).dropna().iloc[-1])
            if not info.get("currentPrice"): info["currentPrice"] = lc
            if not info.get("regularMarketPrice"): info["regularMarketPrice"] = lc
            if not info.get("fiftyTwoWeekHigh") and "High" in hist.columns:
                h = hist["High"]
                if isinstance(h, pd.DataFrame): h = h.iloc[:,0]
                info["fiftyTwoWeekHigh"] = float(h.max())
            if not info.get("fiftyTwoWeekLow") and "Low" in hist.columns:
                l = hist["Low"]
                if isinstance(l, pd.DataFrame): l = l.iloc[:,0]
                info["fiftyTwoWeekLow"] = float(l.min())
        except Exception:
            pass
...
def scan_sector(sector: str) -> dict:
    ...
    for t in tickers:
        ...
        if hist is not None and not hist.empty and "Close" in hist.columns:
            ...
            lc = float(close_col.dropna().iloc[-1])
            if not info.get("currentPrice"): info["currentPrice"] = lc
...
def get_macro_data() -> dict:
    ...
    for label, meta in MACRO_SYMBOLS.items():
        ...
        for attempt in range(3):
            try:
                df = _normalize_hist(yf.download(sym, period="5d", interval="1d",
                                  auto_adjust=True, progress=False,
                                  threads=False, multi_level_index=False))
                if not df.empty and "Close" in df.columns and len(df) >= 2:
                    close_s = df["Close"]
                    if isinstance(close_s, pd.DataFrame): close_s = close_s.iloc[:,0]
                    close_s = close_s.astype(float).dropna()
                    if len(close_s) >= 2:
                        last = float(close_s.iloc[-1]); prev = float(close_s.iloc[-2])
...
def get_sector_etf_perf() -> dict:
    ...
    for sname, sdata in SECTORS.items():
        ...
        for attempt in range(3):
            try:
                df = _normalize_hist(yf.download(etf, period="5d", interval="1d",
                                  auto_adjust=True, progress=False,
                                  threads=False, multi_level_index=False))
                if not df.empty and "Close" in df.columns and len(df) >= 2:
                    close_s = df["Close"]
